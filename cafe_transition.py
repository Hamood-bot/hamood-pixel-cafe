...
# Audio ducking/boost configuration
# When coffee starts, temporarily lower background audio so coffee.mp3 is clearly heard
DUCK_ON_COFFEE = True
DUCK_FACTOR = 0.25   # multiply channel volumes by this while coffee + ring are playing
# Optional: play the coffee sound on two channels at once for perceived loudness boost
COFFEE_DUAL_CHANNEL_BOOST = True
...
class CafeApp:
    def __init__(self, root):
        ...
        try:
            ...
            # Preload ring to know its duration for safe unducking
            if os.path.isfile(RING_SOUND):
                try:
                    self.ring_sound = pygame.mixer.Sound(RING_SOUND)
                    # pygame Sound doesn't directly expose length; use get_length()
                    self.ring_length = self.ring_sound.get_length()
                    self.ring_loaded = True
                except Exception as re:
                    print("[WARN] Could not load ring sound:", re)
    ...
    def play_ring(self):
        try:
            if getattr(self, 'ring_loaded', False):
    ...
    def duck_audio_for_coffee(self):
        if not DUCK_ON_COFFEE or self.is_ducked:
            return
        try:
            # Reduce background channels
            if self.music_channel:
                self.music_channel.set_volume(DUCK_FACTOR)
            if self.rain_channel:
                self.rain_channel.set_volume(DUCK_FACTOR)
            if self.fireplace_channel:
                self.fireplace_channel.set_volume(DUCK_FACTOR)
            self.is_ducked = True
        except Exception as e:
            print('[WARN] Ducking failed:', e)
    ...
    def unduck_audio(self):
        ...
        try:
            ...
            if self.rain_channel:
                self.rain_channel.set_volume(1.0)
            if self.fireplace_channel:
                self.fireplace_channel.set_volume(1.0)
            self.is_ducked = False
        except Exception as e:
            print('[WARN] Unducking failed:', e)
            ...
            if self.coffee_brew_timer >= 5.0:
                ...
                if self.coffee_channel:
                    try:
                        ...
                        pass
                    self.coffee_channel = None
                if getattr(self, 'coffee_channel_2', None):
                    try:
                        self.coffee_channel_2.stop()
                    except Exception:
                        pass
                    self.coffee_channel_2 = None
                # Play ring and transition to coffee scene
                self.play_ring()
                # Schedule unduck after ring
                unduck_delay_ms = int(max(0.3, getattr(self, 'ring_length', 0.8)) * 1000)
                try:
                    self.root.after(unduck_delay_ms, self.unduck_audio)
                except Exception:
                    # If scheduling fails for any reason, try immediate unduck soon after
                    self.unduck_audio()
    ...
    def start_coffee_scene(self):
        ...
        self.coffee_brew_timer = 0
        # Duck other audio while coffee + ring are active
        self.duck_audio_for_coffee()
        # Play coffee brewing sound
        if self.coffee_loaded:
            try:
                if COFFEE_DUAL_CHANNEL_BOOST:
                    # Play on two channels for perceived loudness
                    self.coffee_channel = self.coffee_sound.play(loops=0)
                    self.coffee_channel_2 = self.coffee_sound.play(loops=0)
                else:
    ...
    def get_random_coffee_reading(self):
        ...
        quotes = [
            # Fresh: short, funny, punchy
            "Ship it while it's still fun",
            "Confidence is a habit you can practice",
            "Make drafts, not drama",
            "If it’s important, schedule it. If it’s scheduled, do it",
            "Today called—it wants momentum",
            "Perfect is late. Shipped is useful",
            "Tiny progress beats loud intentions",
            "You can’t edit a blank page",
            "Silence the tabs, amplify the focus",
            "Be the person your commit history thinks you are",
            "Meetings declined, results inclined",
            "Stay scrappy, stay kind",
            "Done is a power-up",
            "Playful energy, serious output",
            "Less scroll, more soul",
            "Out-plan the panic",
            "Boredom is a feature. Use it",
            "Elegance is ruthless about the unnecessary",
            "The shortcut is consistent work",
            "Your taste is trying to help you",
            "Say no like you mean yes to your priorities",
            "Keep the scope small and the standards high",
            "Clarity is kind to future you",
            "Calm is a performance enhancer",
            "Snooze the noise, ship the signal",
            "Go where your curiosity pays rent",
            "Hard choices, easy days",
            "You don’t need perfect conditions, just a start",
            "Turn fear into a to-do list",
            "Focus is a yes that cancels a thousand maybes",
            "Try it messy. Improve it proudly",
            "You are one small task from momentum",
            "Optimize for delight and delivery",
            "Document like a friend will read it",
            "Refactor your expectations, not your joy",
            "Low drama, high bandwidth",
            "Win the morning, coast the evening",
            "Deadlines are decisions wearing a clock",
            "Make the boring parts beautiful",
            "Protect the craft from the chaos",
            "Compounding happens when you stick around",
            "Be allergic to avoidable complexity",
            "Keep it sharp, keep it humane",
            "Your attention is expensive—spend it wisely",
            "Default to kind. escalate to clear",
            "Take the stairs in your brain",
            "Caffeine helps, consistency wins",
            "Courage is a ten-second habit",
            "If it scares you and it’s yours, walk toward it",
            "Never miss twice",
            "Let the small win open the big door",
            "Taste before speed",
            "Start ugly, end proud",
            "Stop negotiating with procrastination",
            "Make the task a game you can win today",
            "When in doubt, reduce the scope",
            "You’re not behind; you’re early for the next version",
            "Build the thing you wish existed",
            "Say less, show progress",
            "Momentum is motivation’s older sibling",
            "Keep promises to yourself in pencil—and then trace in ink",
            "Your best work is bored with your excuses",
            "Delete one thing and do the rest better",
            "Quiet ambition, loud results",
            "Small sprints, generous rests",
            "Make it obvious, then make it lovely",
            "Be useful to your future self",
            "Work like a scientist: test, learn, iterate",
            "Respect your energy budget",
            "Talent is common. Patience is rare",
            "Less friction, more fiction—write the story",
            "Turn comparison into inspiration",
            "Make the default choice the healthy one",
            "If you can’t start, shrink it",
            "Don’t chase motivation; build momentum",
            "Protect your bandwidth like it pays bills",
            "Craft is kindness to your audience",
            "Ambition ages well with boundaries",
            "Sprints end. Systems last",
            "The magic trick is showing up",
            "If it’s not a clear yes, it’s a no",
            "Less clever, more clear",
            "Write the README before the code",
            "Focus turns time into quality",
            "Bet on boring habits",
            "Queue up tomorrow before you log off",
            "Commit to the next tiny version",
            "Make work that makes you lighter",
            "You can be kind and relentless",
            # Medium-length: motivational, friendly, witty
            "Stop waiting for the mood to start. Begin, and the mood will find you busy and decide to stay",
            "Discipline isn’t punishment; it’s an agreement with your future self that you’ll be there when it counts",
            "The day gets easier when the first thing you do is the thing you promised to do",
            "Excellence is almost always quiet: a string of kept promises and a thousand tiny edits",
            "Your standards are not the enemy—unrealistic timelines are. Keep the first, stretch the second",
            "When everything feels chaotic, look for the smallest lever you can pull that makes other things easier",
            "A clear boundary is a love note to your attention. Guard it like it guards your dreams",
            "Finish something small today. Tomorrow respects finishers",
            "If it feels overwhelming, you’ve combined steps. Separate them and walk",
            "You don’t need louder goals—you need quieter distractions",
            "Mastery is being happily bored by the basics and doing them anyway",
            "Let your curiosity do the heavy lifting—point it at useful problems and follow the thread",
            "Professional is just amateur, repeated with care and notes",
            # Longer: reflective and motivating
            "You will not always feel brave, but you can always behave bravely for a moment. That moment is where momentum begins—take the first step, then the next",
            "Do not underestimate the power of plain, boring consistency. It is humility in action and the most reliable engine of remarkable work",
            "When you’re stuck, don’t push harder—remove resistance. Make the next action smaller, clearer, and kinder to your current energy",
            "The version of you that keeps promises to yourself is built one decision at a time. Make a small honest one today and let it compound",
            "Most breakthroughs don’t feel like fireworks. They feel like quiet clarity after a good night’s sleep and a simple plan you actually follow",
        ]
